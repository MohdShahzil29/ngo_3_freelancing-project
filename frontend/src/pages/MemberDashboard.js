...
const MemberDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 py-12" data-testid="member-dashboard">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <Card data-testid="total-donations-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-stone-900">₹{totalDonated.toLocaleString()}</div>
            </CardContent>
          </Card>

          <Card data-testid="donations-count-card">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-stone-600">Donations Count</CardTitle>
              <FileText className="text-secondary" size={20} />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-stone-900">{donations.length}</div>
            </CardContent>
          </Card>

          <Card data-testid="certificates-card">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-stone-600">Certificates</CardTitle>
              <Award className="text-primary" size={20} />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-stone-900">{certificates.length}</div>
            </CardContent>
          </Card>
        </div>
            ...
            <Card>
              ...
              <CardContent>
                {certificates.length === 0 ? (
                  <p className="text-stone-600 text-center py-8">No certificates yet</p>
                ) : (
                  <div className="space-y-4">
                    {certificates.map((cert, index) => (
                      ...
                      >
                        <div>
                          <p className="font-semibold text-stone-900">{cert.certificate_type}</p>
                          <p className="text-sm text-stone-600">
                            Issued: {new Date(cert.issue_date).toLocaleDateString()}
                          </p>
                          <p className="text-sm text-stone-500">Cert No: {cert.certificate_number}</p>
                        </div>
