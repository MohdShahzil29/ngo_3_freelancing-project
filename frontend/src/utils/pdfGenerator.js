...
export const generateCertificatePDF = (certificate) => {
  ...
  doc.text(`of ${certificate.certificate_type}`, 148.5, 55, { align: 'center' });
  ...
  // Certificate details
  doc.setFontSize(10);
  doc.text(`Certificate Number: ${certificate.certificate_number}`, 148.5, 125, { align: 'center' });
  doc.text(`Issue Date: ${new Date(certificate.issue_date).toLocaleDateString()}`, 148.5, 135, { align: 'center' });
  ...
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text('नारायण निवास बजरंग नगर मोड़ा बालाजी रोड दौसा राजस्थान', 148.5, 167, { align: 'center' });
  doc.text('Phone: 78776 43155 | Email: info@nvpwelfare.in', 148.5, 173, { align: 'center' });
  ...
  // Footer note
  doc.setFontSize(8);
  doc.text('This is a computer-generated certificate and does not require a signature.', 148.5, 185, { align: 'center' });
...
export const generateReceiptPDF = (receipt) => {
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.text('नारायण निवास बजरंग नगर मोड़ा बालाजी रोड दौसा राजस्थान', 105, 30, { align: 'center' });
  ...
  if (receipt.receipt_type === 'donation') {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('This donation is eligible for 80G tax benefits', 105, 165, { align: 'center' });
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.text('Please retain this receipt for tax filing purposes', 105, 171, { align: 'center' });
    doc.setTextColor(0, 0, 0);
  }
  ...
  doc.text('Contact: 78776 43155 | Email: info@nvpwelfare.in', 105, contactY, { align: 'center' });

  // Footer
  doc.setFontSize(8);
  doc.setTextColor(128, 128, 128);
  doc.text('This is a computer-generated receipt and does not require a signature.', 105, contactY + 10, { align: 'center' });
  doc.text('For queries, please contact us at the above details.', 105, contactY + 15, { align: 'center' });

  // QR Code placeholder note
  doc.setFontSize(9);
  doc.setTextColor(0, 0, 0);
  doc.text('Scan QR code to verify: Visit nvpwelfare.in/verify', 105, contactY + 25, { align: 'center' });
