...
# -------------------- Env / DB / External clients --------------------
# MongoDB connection
MONGO_URL = os.environ.get("MONGO_URL")
DB_NAME = os.environ.get("DB_NAME", "nvp_db")
if not MONGO_URL:
...
# Default origins - adjust via CORS_ORIGINS env var (comma separated)
# NOTE: If you use cookies/credentials, DON'T use "*" in allow_origins
origins_env = os.environ.get(
    "CORS_ORIGINS",
    "http://localhost:3000,http://localhost:5173,https://ngo-3-freelancing-project-ye1a.vercel.app,https://ngo-3-freelancing-project.onrender.com"
)
origins = [o.strip() for o in origins_env.split(",") if o.strip()]

app.add_middleware(
    CORSMiddleware,
    allow_origins=origins,       # specific or ["*"] for debug (not with credentials)
    allow_credentials=True,      # set True only if you actually use cookies/credentials
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["*"],
    max_age=3600,
...
# Optional: lightweight middleware that logs preflight headers (non-blocking)
@app.middleware("http")
async def log_preflight(request: Request, call_next):
    try:
        if request.method == "OPTIONS":
            logger.info("Preflight OPTIONS received. Headers: %s", dict(request.headers))
    except Exception as e:
        logger.exception("Error in preflight logger: %s", e)
    return await call_next(request)
...
async def register(user_data: UserRegister):
    ...
    html_content = f"<h2>Welcome!</h2><p>Dear {user.name}, your account was created.</p>"
    await send_email(user.email, "Welcome - NVP Welfare Foundation", html_content)
    token = create_jwt_token(user.id, user.email, user.role)
    return {"token": token, "user": {"id": user.id, "name": user.name, "email": user.email, "role": user.role}}
...
async def login(credentials: UserLogin):
    ...
    if not user.get("is_active", True):
        raise HTTPException(status_code=403, detail="Account is blocked")
    token = create_jwt_token(user["id"], user["email"], user.get("role", "member"))
    return {"token": token, "user": {"id": user["id"], "name": user.get("name"), "email": user["email"], "role": user.get("role")}}
...
# -------------------- MEMBERS / DONATIONS / OTHER ROUTES --------------------
# For brevity include same logic as your original file (kept but slightly hardened)
@api_router.post("/members")
async def create_member(member_data: dict, user_data: dict = Depends(verify_token)):
    member_count = await db.members.count_documents({})
    member_number = f"SM{str(member_count + 1).zfill(6)}"
    member = Member(user_id=user_data["user_id"], member_number=member_number, **member_data)
    doc = member.model_dump()
    doc["joined_at"] = doc["joined_at"].isoformat()
    await db.members.insert_one(doc)
    return {"message": "Membership application submitted", "member_number": member_number}
...
async def get_members(user_data: dict = Depends(verify_token)):
    if user_data.get("role") == "admin":
        ...
        members = await db.members.find({"user_id": user_data["user_id"]}, {"_id": 0}).to_list(100)
    return members

@api_router.patch("/members/{member_id}/status")
async def update_member_status(member_id: str, status: str, user_data: dict = Depends(verify_token)):
    if user_data.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Only admins can update status")
    await db.members.update_one({"id": member_id}, {"$set": {"status": status}})
    return {"message": "Status updated"}
...
async def create_donation_order(donation_data: dict):
    if not razorpay_client:
        raise HTTPException(status_code=500, detail="Payment gateway not configured")
    amount_in_paise = int(donation_data["amount"] * 100)
    razor_order = razorpay_client.order.create({"amount": amount_in_paise, "currency": "INR", "payment_capture": 1})
    receipt_number = generate_receipt_number()
    donation = Donation(
        donor_name=donation_data["donor_name"],
        donor_email=donation_data["donor_email"],
    ...
    doc["created_at"] = doc["created_at"].isoformat()
    await db.donations.insert_one(doc)
    return {"order_id": razor_order["id"], "amount": amount_in_paise, "currency": "INR", "donation_id": donation.id}
...
async def verify_donation_payment(payment_data: dict):
    ...
    if not donation:
        raise HTTPException(status_code=404, detail="Donation not found")
    await db.donations.update_one({"order_id": payment_data["order_id"]}, {"$set": {"status": "completed", "payment_id": payment_data["payment_id"]}})
    qr_data = f"https://starmarketing.in/verify-receipt/{donation['receipt_number']}"
    qr_image = generate_qr_code(qr_data)
    html_content = f"""
    <h2>Thank You for Your Donation!</h2>
    ...
    """
    await send_email(donation["donor_email"], "Donation Receipt - NVP", html_content)
    return {"message": "Payment verified and receipt sent", "receipt_number": donation["receipt_number"]}

# ... (Include other routes such as certificates, news, activities, campaigns, enquiries,
# beneficiaries, events, projects, internships, designations, receipts, stats, uploads etc.)
# For brevity you can re-use your previous route implementations here exactly as needed.
# Make sure each route uses `api_router` decorator like above and uses safe checks.

# Example basic upload route (kept)
UPLOAD_DIR = ROOT_DIR / "uploads"
UPLOAD_DIR.mkdir(exist_ok=True)
